...
float _StaticSobelMultiplier = 1.0f;
float _StaticSobelPower = 1.0f;
float _SobelInvalidateStrength = 1.0f;

StructuredBuffer<float> _SobelMultiplier;
...
void SobelFilter(uint3 id, uint3 GroupId, uint3 GroupThreadId, uint GroupIndex)
{
    ...
    float sobel = centralValue > 0.0 ? min(abs(sobel_edge_h) + abs(sobel_edge_v), _SobelInvalidateStrength) : _SobelInvalidateStrength; // approximation without sqrt

    float edge = _SobelMultiplier[id.z] * pow(sobel, _StaticSobelPower) * _StaticSobelMultiplier;

    // invalidate edge border
    float invalidateEdgeWidthPixels = _SobelInvalidateEdgeWidth * min(_MaskTextureTS.z, _MaskTextureTS.w) * 0.5;
    float2 oneOverEdgeWidth = 1.0f / invalidateEdgeWidthPixels;
    float2 edgeBorder = saturate((float2(invalidateEdgeWidthPixels, invalidateEdgeWidthPixels) - float2(id.xy)) * oneOverEdgeWidth) +
        saturate((float2(id.xy) - (_MaskTextureTS.zw - float2(invalidateEdgeWidthPixels, invalidateEdgeWidthPixels) - 1.0f)) * oneOverEdgeWidth);
    float borderMask = min(pow(edgeBorder.x + edgeBorder.y, 2.0), 1.0);
    float outputValue = max(edge, borderMask * (_SobelMultiplier[id.z] * 2.0 + 10.0));

    _MaskTexture[DK_SOBEL_FILTER_OUTPUT_COORD(id)] = centralValue > 0.0 ? outputValue : max(outputValue, 1.0);
}